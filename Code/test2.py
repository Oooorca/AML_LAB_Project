...
# 创建保存图像的文件夹
PLOT_DIR = "AML_LAB_Project\Plots"
...
def convert_to_angle(sensor_values, min_value=10, max_value=250, max_angle=160):
    """
    将传感器原始值转换为弯曲角度 (°)。
    :param sensor_values: numpy 数组或列表
    :param min_value: 传感器最小读数
    :param max_value: 传感器最大读数
    :param max_angle: 最大弯曲角度
    :return: 弯曲角度数组
    """
    sensor_values = np.clip(sensor_values, min_value, max_value)
    angle = (max_value - sensor_values) / (max_value - min_value) * max_angle
    return angle


def visualize_time_series(df, grasp_folders, sample_size=100, max_time=1500):
    """
    修改版时序可视化：
    - 转换纵坐标为弯曲角度 (°)
    - 固定横坐标范围为 0 ~ 1500 ms
    """
    fig, axs = plt.subplots(len(grasp_folders), 1, figsize=(12, 16), sharex=True)
    if len(grasp_folders) == 1:
        axs = [axs]  # 确保 axs 可迭代

    for i, grasp_name in enumerate(grasp_folders):
        subset = df[df['label'] == grasp_name]
        subset_sample = subset.sample(min(sample_size, len(subset)), random_state=42).sort_values(by='time')

        # 将时间归一化到 0 ~ max_time
        times = np.linspace(0, max_time, len(subset_sample))

        # 转换为角度
        for col in ['flex1', 'flex2', 'flex3', 'flex4', 'flex5', 'flex6']:
            angles = convert_to_angle(subset_sample[col].values)
            axs[i].plot(times, angles, label=col)

        axs[i].set_title(f"Time Series of {grasp_name} (Angle in °)")
    ...
    # 保存图像
    save_path = os.path.join(PLOT_DIR, "time_series_with_angles.png")
